...
define("site_dir", get_template_directory_uri());
 ...
 add_action( 'woocommerce_thankyou', 'code_for_payment',1,1 );
...
function twentytwenty_register_styles() {
	...
	wp_enqueue_style( 'twentytwenty-style-additional', get_template_directory_uri() . '/css/style.css', null, $theme_version );
...
function twentytwenty_sidebar_registration() {
	...
	register_sidebar(
		array_merge(
			$shared_args,
			array(
				'name'        => __( 'Footer Full Width', 'twentytwenty' ),
				'id'          => 'footer-fullwidth-1',
				'description' => __( 'Widgets in this area will be displayed in the in the footer.', 'twentytwenty' ),
			)
		)
	);
...
function custom_override_checkout_fields( $fields ) {
  ...
  $fields['shipping']['shipping_address_1']['label'] = "Full Address";

  $fields['billing']['billing_city']['label'] = "Emirate";
...
function my_custom_checkout_field_display_admin_order_meta($order){
    ...
    $awb = get_post_meta( $order->id, 'Fetchr AWB Link', true );

    if(count($awb)>1){

      echo '<p><strong>'.__('AWB').':</strong> ' . $awb . '</p>';
      echo '<p><strong>'.__('Tracking Number').':</strong> ' . get_post_meta( $order->id, 'Fetchr Tracking No', true ) . '</p>';

    }

    $date =  get_post_meta( $order->id, 'Delivery Date', true );
    $date = date("j M Y", strtotime($date));
    echo '<p><strong>'.__('Preferred Meetup Date').':</strong> ' . $date . '</p>';
}
...
function my_custom_checkout_field( $checkout ) {
    ...
    echo '<div id="my_custom_checkout_field"><strong>' . __('Preferred Meetup/Delivery Date') . '</strong>';
...
function my_custom_checkout_field_update_order_meta( $order_id ) {
    if ( ! empty( $_POST['deliverydate'] ) ) {
        update_post_meta( $order_id, 'Delivery Date', sanitize_text_field( $_POST['deliverydate'] ) );
    }
}
